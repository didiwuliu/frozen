...
function $LTweenLiteChild($target,$duration,$vars){
    var s = this,k=null;
    s.target=$target;s.duration=$duration || 1;s.vars=$vars;
    ...
    if(s.vars.params){
        s.params = s.vars.params;
        delete s.vars.params;
    }
    if(s.vars.onUpdate){
...
$LTweenLiteChild.prototype = {
    ...
    tween:function(){
        var s = this;
        var time = (new Date()).getTime(),etime;
        etime = (time - s.startTime);
        if(etime < 0)return;

        if(etime >= s.duration && s.yoyo){
        ...
        for(tweentype in s.varsto){
            var v = s.ease(etime/1000,s.varsfrom[tweentype],s.varsto[tweentype]-s.varsfrom[tweentype],s.duration/1000);
            s.target[tweentype] = v;
        }
        if (etime >= s.duration){
            for(tweentype in s.varsto)s.target[tweentype] = s.varsto[tweentype];
            if(s.onComplete)
                s.onComplete(s.params);
            return true;
        }else if(s.onUpdate){
            s.onUpdate(s.target);
        }
...
$LTweenLite.prototype = {
    ...
    frame:function(){
        ...
        for(i=0;i < length;i++){
            //防止在s.tweens[i].tween里改变了s.tweens，导致s.tweens.splice(i,1)要删除的不是原来的第i个
            var tween = s.tweens[i];
            if(!tween)continue;
            if(tween.tween()){
                ...
                if(s.tweens[i] == tween){
                    ...
                    tween.tween = function(){return true};
                }
            }
        }
